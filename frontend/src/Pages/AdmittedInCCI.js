    ...
    async function fetchData() {
      try {
        const p = await axios.get(
          "http://localhost:4000/api/get-admittedInCCI"
        );
        console.log(p.data);
  ...
  const handleDelete = async (e) => {
    ...
    try {
      const pp = await axios.post(
        "http://localhost:4000/api/delete-admittedInCCI",
        { nameD }
      );
      console.log(pp);
      index = 0;
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const po = await axios.post(
        "http://localhost:4000/api/add-admittedInCCI",
        { name, type, step, part, num, description: desc }
      );
      console.log(po);
      index = 0;
      fetchData();
